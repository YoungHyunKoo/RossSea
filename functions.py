...
def retrieve_ERA5_monthly(year, region = "NH"):
    c = cdsapi.Client()
    ...
    if region == "NH":
        params = {
            ...
            'variable': ['10m_u_component_of_wind', '10m_v_component_of_wind', '2m_temperature', 'sea_ice_cover',
                         ...
                         'large_scale_snowfall', 'snow_density', 'snow_depth', 'snowfall'],
            'year':[str(year)],
            'month': ['01', '02', '03', '04', '05', '06','07', '08', '09','10', '11', '12'],
            'time': '00:00',
            'grid': [1, 0.5],
...
def get_ERA5(ds, i, xx, yy, region = "NH"):
    ...
    sp = np.array(ds.sp[i]).transpose()
    sst = np.array(ds.sst[i]).transpose()
    msl = np.array(ds.msl[i]).transpose()
    ...
    grid_sp = griddata((xx3.flatten(), yy3.flatten()), np.array(sp).flatten(), (xx, yy), method='linear') / 100
    grid_msl = griddata((xx3.flatten(), yy3.flatten()), np.array(msl).flatten(), (xx, yy), method='linear') / 100
    grid_sst = griddata((xx3.flatten(), yy3.flatten()), np.array(sst).flatten(), (xx, yy), method='linear')
    
    # grid_t2m[np.isnan(grid_t2m)] = 0
    # grid_u10[np.isnan(grid_u10)] = 0
    # grid_v10[np.isnan(grid_v10)] = 0
    # grid_sic[np.isnan(grid_sic)] = 0
    # grid_sp[np.isnan(grid_sp)] = 0
    # grid_sst[np.isnan(grid_sst)] = 0
    # grid_msl[np.isnan(grid_msl)] = 0
    
    return grid_t2m, grid_u10, grid_v10, grid_sic, grid_sp, grid_sst, grid_msl
